...
var page=1;
var limit = 5
var orderString = "product_typeID";
var term = "";
var DateFrom = "";
var DateTo = "";
var state="";
...
myHeaders.append("Authorization", "Bearer "+localStorage.getItem('token'));
  var url = "https://localhost:7168/api/product_types/getPages";
var options = {
  method: 'GET',
  headers:myHeaders,
 // redirect: 'follow',
...
async function getData(url,options,page,section,limit,orderString) {
    try {
        ...
        +limit+"&&term="+term+"&&state="+state+"&&DateFrom="+DateFrom+"&&DateTo="+DateTo,options);
return res;
...
async function viewData(page,section,limit){
  ...
  if(result.status == 200){
       var productTypes = await result.json()
 if(productTypes._data.length > 0){
  document.getElementById("loader").style.display="none"
drowTable(productTypes._data)
pagenation(productTypes._pageList,productTypes._hasNext,productTypes._hasPrev,productTypes._limit,section,next,prev)
setLable(productTypes._display_from,productTypes._display_to,productTypes._total)
 }else{
...
async function deleteUser(id){
  let myHeaders = new Headers();
  myHeaders.append("Authorization", "Bearer "+localStorage.getItem('token'));
  var url = "https://localhost:7168/api/product_types/delete/"+id
  var options= {
  method: 'Delete',
  headers:myHeaders,
 // redirect: 'follow',
...
case("update-td"):{
  var url = "https://localhost:7168/api/product_types/get/"+id
  let type = getById(url)
  .then(res=>{
    console.log(res)
    _('utypeID').value=id
    _('utype').value=res.name
...
document.getElementById('update_type_frm').addEventListener('submit',async function(e){
...
_('update_button').onclick=async function(e)
  {
  e.preventDefault()
   if(_('utypeID').value == null || _('utypeID').value== "undefined"){
alert("لقد حدث خطأ!!")
}else{
var state = document.getElementById("u_active").checked ? document.getElementsByName("ustate")[0].value: document.getElementsByName("ustate")[1].value;
   ...
   var url = "https://localhost:7168/api/product_types/update"
    let result = await updateType(url,row);
    if(result.status == 200){

show_massege(_('uresult'),'لقد تم التعديل بنجاح','alert-success')
     viewData(page,section,limit)
   }else{
      show_massege(_('uresult'),'عذرا لقد حدث خطأ','alert-danger')
...
{
e.preventDefault()
var url = "https://localhost:7168/api/product_types/add";
...
var row = JSON.stringify({
  "Name": _('Name').value,
  "state":state,
  "product_type_userID":localStorage.getItem('userId')
});
  //url = "https://localhost:7168/api/users/add"
let result = await addProductType(url,row)
console.log(result)
result ? show_massege(document.getElementById('result'),"لقد تمت العملية بنجاح","alert-success") 
: show_massege( show_massege(document.getElementById('result'),"لقد حدث  خطأ","alert-danger"))
viewData(page,section,limit)
...
_('pdf_export').addEventListener('click',async function(e){
  e.preventDefault()
  var headers = { id: "الصنف", name: "الحالة"}
